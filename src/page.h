          ...
          tx_context_cache(1000)
{
    ...
    template_file["quorum_states_full"]  = get_full_page(xkreg::read(TMPL_QUORUM_STATES));
    ...
    template_file["tx_details"]      = xkreg::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
    template_file["tx_table_header"] = xkreg::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
    template_file["tx_table_row"]    = xkreg::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    if (CurrentBlockchainStatus::is_thread_running())
    {
        ...
        string emission_coinbase = xkr_amount_to_str(current_values.coinbase, "{:0.3f}");
        string emission_fee      = xkr_amount_to_str(current_values.fee, "{:0.3f}");

        context["emission"] = mstch::map {
                {"blk_no"    , emission_blk_no},
                {"amount"    , emission_coinbase},
                {"fee_amount", emission_fee},
                {"circulating_supply", xkreg::make_comma_sep_number(CurrentBlockchainStatus::circulating_supply)}
        };
...
show_service_node(const std::string &service_node_pubkey)
{
    ...
    else
    {
      ...
      snprintf(buf, sizeof(buf),
          "This service node is awaiting to be registered and has: %s kredits to be contributed remaining",
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_xkri_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
{
    ...
    mstch::map context {
            {"xkr_address"        , REMOVE_HASH_BRAKETS(address_str)},
...
json_networkinfo()
{
    ...
    if (!get_xkri_network_info(j_info))
    {
        j_response["status"]  = "error";
...
json_emission()
{
    ...
    else
    {
        ...
        string emission_blk_no   = std::to_string(current_values.blk_no - 1);
        string emission_coinbase = xkr_amount_to_str(current_values.coinbase, "{:0.3f}");
        string emission_fee      = xkr_amount_to_str(current_values.fee, "{:0.4f}", false);
...
json_version()
{
    ...
    j_data = json {
            ...
            {"xkri_version_full"   , string {KREDITS_VERSION_FULL}},
...
bool
get_xkri_network_info(json& j_info)
{
